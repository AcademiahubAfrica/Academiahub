...
/**
 * Reusable newsletter form. The backend stores subscribers through POST /api/subscribe.
 */
export default function SubscribeForm({ className }: { className?: string }) {
  ...
  const onSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      try {
        // Keep the backend request here so the footer remains a presentational component.
        ...
        if (!res.ok) {
          // Prefer the backend's message so users receive a specific error, e.g. duplicates.
