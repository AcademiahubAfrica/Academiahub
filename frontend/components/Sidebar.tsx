...
const Sidebar = () => {
  ...
  return (
    <aside
      className={`h-screen fixed top-0 left-0 transition-all duration-300 w-12 lg:w-62.5 overflow-hidden hover:overflow-y-auto lg:border-r bg-white shadow-lg p-5  ${isExpanded ? "md:w-62.5 z-50" : "md:w-12"}`}
    >
      <Logo />

      <MdOutlineKeyboardArrowRight
        size={24}
        className="hidden text-3xl md:block lg:hidden absolute top-6 right-1 cursor-pointer transition-transform duration-300"
      ...
      />
      <div className="flex flex-col justify-between h-full mt-3.75 pb-4">
        <div>
          <nav>
